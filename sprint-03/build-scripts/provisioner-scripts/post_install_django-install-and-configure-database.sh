...
##############################################################################################
# CHANGE THE VALUES ~/2021-team03t TO YOUR TEAM REPO AND ADJUST THE PATH ACCORDINGLY     #
...
sed -i "s/\$ACCESSFROMIP/$ACCESSFROMIP/g" ~/2021-team03t/sprint-03/code/db/*.sql
sed -i "s/\$USERPASS/$USERPASS/g" ~/2021-team03t/sprint-03/code/db/*.sql
sed -i "s/\$DATABASENAME/$DATABASENAME/g" ~/2021-team03t/sprint-03/code/db/*.sql
##############################################################################################
...
sudo mysql -u root < ~/2021-team03t/sprint-03/code/db/create-database.sql
# This script will create the non-root user named worker and the user for replication
sudo mysql -u root < ~/2021-team03t/sprint-03/code/db/create-user-with-permissions.sql
