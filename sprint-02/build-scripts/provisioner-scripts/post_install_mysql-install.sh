...
###############################################################################
# Replace any occurance of 2021-team03t with the name of your own team private repository #
###############################################################################

sudo chown -R vagrant:vagrant ~/2021-team03t

# Using sed to replace variables in the scripts with the ENV variables passed
sed -i "s/\$ACCESSFROMIP/127.0.0.1/g" ~/2021-team03t/sprint-02/code/db-samples/*.sql
sed -i "s/\$USERPASS/$USERPASS/g" ~/2021-team03t/sprint-02/code/db-samples/*.sql
sed -i "s/\$MMIP/127.0.0.1/g" ~/2021-team03t/sprint-02/code/db-samples/*.sql
...
sudo mysql -u root < ~/2021-team03t/sprint-02/code/db-samples/create-user-with-permissions-mm.sql
...
sudo mysql -u root < ~/2021-team03t/sprint-02/code/db-samples/sample-select.sql
